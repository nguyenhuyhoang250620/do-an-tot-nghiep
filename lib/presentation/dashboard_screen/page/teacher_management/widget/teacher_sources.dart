...
class TeacherDataTableSource extends DataTableSource {
  ...
  DataRow getRow(int index) {
    ...
    return DataRow.byIndex(
      index: index,
      cells: [
        DataCell(labelAndAvt(index, row['TenGV'].toString(),
            row['MaGV'].toString(), row['url'].toString())),
...
Widget buildAuthorcation(
  ...
  return Container(
    ...
    child: Container(
      decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(4.0), color: orange),
      child: Padding(
...
Widget alertAuthor(String ma_giang_vien) {
  final controller = Get.find<TeacherController>();
  List filterListOnly = [];
  List finalList = [];
  ...
  return AlertDialog(
          ...
          children: [
            ...
            Container(
              padding: EdgeInsets.symmetric(
                  vertical: appPadding, horizontal: appPadding * 4),
              height: 500,
                ...
                itemBuilder: (context, index) {
                  return Obx(
                    () => CheckboxListTile(
                      ...
                      onChanged: (newValue) {
                        controller.listGetAuthor[index].key.value =
                            newValue ?? false;
                        if (controller.listGetAuthor[index].key.value) {
                          filterListOnly
                              .add(controller.listGetAuthor[index].ma_quyen);
                          finalList = filterListOnly
                          ...
                          finalList.removeWhere((element) =>
                              element ==
                              controller.listGetAuthor[index].ma_quyen);
                        }
                      },
    ...
    actions: [
      CustomButtonAlert(
          ...
          onPressedEnable: () {
            Get.back();
            controller.authorPermission(
                ma_giang_vien, controller.role.value, finalList);
          })
    ],
  );
}
                  ...
                  children: [
                    Center(
                      child: Padding(
                        ...
                        child:
                            Text('Ảnh 3x4', style: AppStyle.txtInterRegular18),
                      ),
                    ),
                    Container(
                      height: 300,
                      ...
                      child: Obx(
                        () {
                          ...
                          return controller.pathImageAfterPickfileFromComputer
                                  .value.isNotEmpty
                              ? Image.network(
                                  controller
                                      .pathImageAfterPickfileFromComputer.value,
      ...
      CustomButtonAlert(
          ...
          onPressedEnable: () {
            controller.uploadAvatar(
                ma_giang_vien, controller.pickFileFromComputer);
            Get.back();
            Get.dialog(Dialog(
              child: CustomLoading(),
            ));
