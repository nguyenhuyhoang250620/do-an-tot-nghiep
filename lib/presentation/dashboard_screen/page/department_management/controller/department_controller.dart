...
class DepartmentController extends GetxController {
  ...
  Future<void> updateDepartment(
    ...
    await apiClient.updateDepartment(ten_don_vi, ma_don_vi, mo_ta).whenComplete((){
      dashBoardController.getDepartment();
    });
  }

  //xoa phong ban
  Future<void> deleterDepartment(
    String ma_don_vi,
  ) async {
    await apiClient.deleteDepartment(ma_don_vi).whenComplete((){
      dashBoardController.getDepartment();
    });
  }
  ...
  //xuat file excel
   Future<void> exportData() async {
    ...
    range3.merge();
    sheet.getRangeByName('A4:D4').cellStyle = globalStyle;
    sheet.getRangeByName('A5:I${dashBoardController.getDepartmentList.length + 4}').cellStyle = globalStyle1;
    sheet.getRangeByIndex(4, 1).setText('STT');
    sheet.getRangeByIndex(4, 2).setText(TenDV.value);
    sheet.getRangeByIndex(4, 3).setText(MaDV.value);
    sheet.getRangeByIndex(4, 4).setText(Mota.value);

    int row = 5;
    for (int i = 0; i < dashBoardController.getDepartmentList.length; i++) {
      int column = 1;
      sheet.getRangeByIndex(row, column).setValue(i);
      column++;
      sheet.getRangeByIndex(row, column).setText(dashBoardController.getDepartmentList[i].TenDV);
      column++;
      sheet.getRangeByIndex(row, column).setText(dashBoardController.getDepartmentList[i].MaDV);
      column++;
      sheet.getRangeByIndex(row, column).setText(dashBoardController.getDepartmentList[i].Mota);
      row++;
    }
